...
BITMAP *norm_image(int *accum, int w, int h, BITMAP *ainit=NULL, BITMAP *mask=NULL) {
  BITMAP *ans = new BITMAP(w, h);
  for (int y = 0; y < h; y++) {
    int *row = (*ans)[y];
    int *arow = NULL;
    ...
    for (int x = 0; x < w; x++) {
      ...
      if (ainit)
        row[x] = p[3] ? int((p[0]+c2)/c)|(int((p[1]+c2)/c)<<8)|(int((p[2]+c2)/c)<<16) : arow[x];
      else
        row[x] = int((p[0]+c2)/c)|(int((p[1]+c2)/c)<<8)|(int((p[2]+c2)/c)<<16);
      //row[x] = p[0] | (p[1] << 8) | (p[2] << 16);
...
void patchmatch(BITMAP *a, BITMAP *mask, BITMAP *&ans, BITMAP *&ann, BITMAP *&annd) {
  ...
  int bx, by;
  // Initialization
  for (int ay = 0; ay < meh; ay++) {
    for (int ax = 0; ax < mew; ax++) {
      bool valid = false;
      while (!valid) {
        bx = rand()%mew;
        by = rand()%meh;
  ...
  for (int iter = 0; iter < pm_iters; iter++) {
    ...
    int* accum = new int[sz];
    memset(accum, 0, sizeof(int)*sz );
